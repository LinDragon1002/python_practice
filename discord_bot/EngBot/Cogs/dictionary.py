...
from Cogs.database import execute_query
...
class DictionaryCog(commands.Cog):
    ...
    async def newvoc(self,
        ...
        sql = "INSERT INTO eng_dictionary (word, translation, abbreviation, definition, example) VALUES (%s, %s, %s, %s, %s)"
        try:
            execute_query(sql,(word, translation, abbreviation, definition, example))
    ...
    async def checkvoc(self, interaction: discord.Interaction, word: str):
        ...
        sql = "SELECT * FROM eng_dictionary WHERE word = %s"
        result = execute_query(sql, (word,), fetchone=True)
        ...
        if result:
            ...
            example = result['example'] if result['example'] else "沒有例句"
    ...
    async def allvoc(self, interaction: discord.Interaction):
        ...
        sql = "SELECT * FROM eng_dictionary"
    ...
    async def delvoc(self, interaction: discord.Interaction, word: str):
        ...
        sql = "DELETE FROM eng_dictionary WHERE word = %s"
        execute_query(sql,(word,))
