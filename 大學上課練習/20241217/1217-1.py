import jieba
with open ('positives.txt','r',encoding='utf-8')as infile:
    positive = infile.readlines()
    print(positive)

with open ('negatives.txt','r',encoding='utf-8')as infile:
    negative = infile.readlines()
    print(negative)

# with open('president.txt','r',encoding='utf-8')as infile:
#     data = infile.read()
#     items = jieba.cut(data)
#     print(list(items))
...
謝謝。
'''
items = list(jieba.cut(data))
print(items)

score = 0
for t in items:
    if t in positive: score+=1
    if t in negative: score-=1
